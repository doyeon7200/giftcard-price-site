...
const pool = mysql.createPool({
  ...
  database: process.env.DB_NAME || "giftcard_db",
...
async function seed() {
  ...
  try {
    // Clear existing data
    await connection.execute("DELETE FROM giftcards");
    
    // Insert new data
    for (let i = 0; i < completeData.length; i++) {
      const item = completeData[i];
      await connection.execute(
        "INSERT INTO giftcards (name, category, sellPrice, sellDiscount, buyPrice, buyDiscount, note, available, displayOrder) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [item.name, item.category, item.sellPrice, item.sellDiscount, item.buyPrice, item.buyDiscount, item.note, item.available ? 1 : 0, i]
      );
    }
